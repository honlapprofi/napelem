    ...
    private function sendOrderEmail($orderRef, $formID) {
        ...
        if(!empty($_POST)) {
            $submissionData = [
                ...
                'roof' => [
                    'ids' => [276, 280, 281, 282, 283, 284, 285, 286],
                    'property' => 'label',
                    'value' => ''
                ],
                'electricityBill' => [
                ...
                'hasThreePhaseGeneral' => [
                    'ids' => [323],
                    'property' => 'value',
                    'value' => ''
                ],
                'roofGeneral' => [
                    'ids' => [324, 325, 326, 327, 328, 329, 330, 331],
                    'property' => 'label',
                    'value' => ''
                ],
                'electricityBillGeneral' => [
                    'ids' => [333],
                    'property' => 'value',
                    'value' => ''
                ],
            ...
            foreach($submissionData as $key => &$itemConfig) {
                foreach($_POST['items'] as $formItem) {
                    if(in_array($formItem['itemid'], $itemConfig['ids'])) {
                        $itemConfig['value'] = $formItem[$itemConfig['property']];
                    }
                }
            }

            if(!empty($submissionData['acListId']['value'])) {
                require_once("ACConnector.php");
                $connector = new ACConnector($submissionData['acListId']['value']);

                $submit = $connector->submitForm([
                    "email" => sanitize_text_field($_POST['email']),
                    "firstName" => sanitize_text_field($_POST['customerInfos']['firstName']),
                    "lastName" => sanitize_text_field($_POST['customerInfos']['lastName']),
                    "phone" => sanitize_text_field($_POST['customerInfos']['phone']),
                    "zip" => sanitize_text_field($_POST['customerInfos']['zip']),
                    "town" => sanitize_text_field($_POST['customerInfos']['city']),
                    "address" => sanitize_text_field($_POST['customerInfos']['address']),
                    "note" => sanitize_text_field($submissionData['note']['value']),
                    "propertyType" => sanitize_text_field($submissionData['propertyType']['value']),
                    "supportUsage" => sanitize_text_field($submissionData['supportUsage']['value']),
                    "hasThreePhase" => sanitize_text_field($submissionData['hasThreePhase']['value']),
                    "roof" => sanitize_text_field($submissionData['roof']['value']),
                    "electricityBill" => sanitize_text_field($submissionData['electricityBill']['value']),
                    "hasThreePhaseGeneral" => sanitize_text_field($submissionData['hasThreePhaseGeneral']['value']),
                    "roofGeneral" => sanitize_text_field($submissionData['roofGeneral']['value']),
                    "electricityBillGeneral" => sanitize_text_field($submissionData['electricityBillGeneral']['value']),
                    "investDateGeneral" => sanitize_text_field($submissionData['investDateGeneral']['value'])
                ]);
            }
        }
        ...
        if (count($rows) > 0) {
            ...
            if ($order->sendToUser && $order->email != '') {
                $attachmentCustomer = [
                    //$this->dir . '/uploads/pdf/'. $order->totalPrice .'.pdf'
                ];
